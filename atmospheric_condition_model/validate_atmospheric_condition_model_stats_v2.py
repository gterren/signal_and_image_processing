...
# Path to the data
path = r'/users/terren/atmospheric_condition_model/data/v8'
...
for idx_degree in range(N_degrees):
    ...
    score_max = np.max(scores_[:, idx_degree], axis = 0)
    idx_complex, idx_degree = np.where(scores_ == score_max)
    C = complex_[idx_complex][0]
    degree = degrees_[idx_degree][0]
    print(score_max, C, degree)
    # Get leave-one-out train Dataset
    W_, R_, N_, S_, M_, Y_ = _get_train_data(X_0_tr_, X_1_tr_, X_2_tr_, X_3_tr_)
    X_tr_, y_tr_ = _get_dataset(W_, R_, N_, S_, M_, Y_)
    ...
    #            name = 'atmospheric_condition_model_v8-TM6_{}.pkl'.format(idx_degree))
